...
const BASE_PRICES = { Rice:2300, Wheat:2400, Maize:2100, Cotton:7200, Soyabean:4600, Onion:1800, Potato:1300, Tomato:2000, Sugarcane:350, 'Bengal Gram(Gram)':5800, Mustard:5400, Groundnut:6300 };
...
async function main(){
  console.log('Deleting old seeds...');
  const d=await prisma.priceHistory.deleteMany({where:{source:'seed'}});
  console.log('Deleted',d.count,'rows. Building new rows...');
  const days=90; const today=new Date(); today.setUTCHours(0,0,0,0);
  const rows=[];
  for(const [commodity,base] of Object.entries(BASE_PRICES)){
    const sea=SEA[commodity]??FLAT; let drift=0;
    for(let i=days;i>=0;i--){
      ...
      for(const [m,mkt] of markets.entries()){
        ...
        rows.push({commodity,priceDate:date,minPrice:modal+off-spread,maxPrice:modal+off+spread,modalPrice:modal+off,unit:'Rs/quintal',source:'seed',...mkt});
      }
    }
  }
  ...
  for(let s=0;s<rows.length;s+=300){
    try{const r=await prisma.priceHistory.createMany({data:rows.slice(s,s+300),skipDuplicates:true});ins+=r.count;}catch(e){console.error('batch err');}
  }
  console.log('Done! Inserted',ins,'rows');
}
